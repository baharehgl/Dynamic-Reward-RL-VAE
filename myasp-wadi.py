...
# Convert to numeric, drop all‐NaN rows/columns
...
# Build an array of shape (num_samples, 25, n_features_full), then drop zero‐var sensors
temp = np.array([
    TS.iloc[i - N_STEPS + 1 : i + 1][feature_cols_full].values
    for i in sample_idx
], dtype="float32")  # → (num_samples, 25, n_features_full)

sensor_var = temp.var(axis=(0,1))  # variance over all sampled windows
...
# Flatten each window of length 25×n_features
windows = np.array([
...
# Scale them—this can produce some huge values if TS contains anomalies,
# so we clip them to [−10,10] to avoid overflow.
...
class EnvWaDi:
    """
    Environment for WADI that keeps a single “current” state of shape (25, n_features+1).
    reset() → one state (25, nf+1)
    step(a) → (two_states, [r0, r1], done, {})
      where two_states is a (2,25,nf+1) array;
      the RL loop picks two_states[a] as the next‐state to feed back into Q‐net.
    """
    ...
    def reset(self):
        self.t = self.t0
        # Precompute flattened “action=0” windows for AL + LP
        self.states_list = []
        ...
        # Return single current state (action=0 branch)
...
def train_and_validate(AL_budget):
    ...
    for ep in range(1, EPISODES + 1):
        ...
        if np.any(labs != -1):
            Warr = np.array(env.states_list)  # (#windows, 25*(n_features+1))
        ...
        # Step(3): Replay updates (5 minibatches)
        for _ in range(5):
            batch = random.sample(memory, min(BATCH_SIZE, len(memory)))
            S_batch, R_batch, NS_batch, _ = map(np.array, zip(*batch))
            qn     = qt.predict(NS_batch, sess)         # (B,2)
            qn_max = np.max(qn, axis=1, keepdims=True)  # (B,1)
...
if __name__ == "__main__":
    # Hyperparameters (you can reduce if it’s still too slow)
    EPISODES   = 10     # RL episodes per AL‐budget
    BATCH_SIZE = 64     # minibatch size for replay
    DISCOUNT   = 0.5
    TN, TP, FP, FN = 1, 10, -1, -10
    NUM_LP     = 200    # how many pseudo‐labels each episode
