...
# in original, raw==1 means no attack → label=0. so normal positions are raw_lbl==1
normal_positions = np.where(lbl_all[N_STEPS:] == 1)[0] + N_STEPS

# sample up to 200 normal windows
MAX_VAE_SAMPLES = 200
sample_idx = np.random.choice(normal_positions,
                              size=min(MAX_VAE_SAMPLES, len(normal_positions)),
                              replace=False)
...
# compute per-step penalty_array over entire series
print("[VAE] computing per-step reconstruction error")
all_windows = np.array([
    df.iloc[i - N_STEPS + 1 : i + 1][feature_cols].values.flatten()
    for i in range(N_STEPS - 1, len(df))
], dtype='float32')
# note: re-fit scaler on all_windows to maintain same scale
...
# pad first N_STEPS-1 entries with zeros
penalty_array = np.concatenate([np.zeros(N_STEPS - 1), recon_err])
print(f"[VAE] raw penalty stats: min={penalty_array.min():.3f}, "
      f"max={penalty_array.max():.3f}, mean={penalty_array.mean():.3f}")
scaled = (penalty_array / (penalty_array.max() + 1e-8)) * 10.0
print(f"[VAE] scaled penalty stats: min={scaled.min():.3f}, "
      f"max={scaled.max():.3f}, mean={scaled.mean():.3f}\n")
...
NUM_LP     = 50      # lowered from 200 → 50
K_SLICES   = 3       # validation slices
INITIAL_COEF = 1.0   # start with a smaller coefficient
...
# state & reward using precomputed, scaled penalty_array
def make_state(ts, c):
    if c < N_STEPS:
        return None
    W = ts[feature_cols].iloc[c - N_STEPS + 1 : c + 1].values.astype("float32")
    s0 = np.concatenate([W, np.zeros((N_STEPS, 1), dtype="float32")], axis=1)
    s1 = np.concatenate([W, np.ones ((N_STEPS, 1), dtype="float32")], axis=1)
...
def reward_fn(ts, c, a, coef):
    ...
    pen = coef * (raw_pen / (penalty_array.max() + 1e-8)) * 10.0
    if np.isnan(pen):
        pen = 0.0
    lbl = ts["label"].iat[c]
...
def train_and_validate(AL_budget):
    ...
    # build Q-networks
    ql = QNet("qlearn"); ql.build()
    qt = QNet("qtarget"); qt.build()
    ...
    # warm-up IsolationForest on a few normal windows
    env.reset()
    W0 = []
    ...
    for ep in range(1, EPISODES + 1):
        ...
        # Replay‐memory updates (5 gradient steps)
        for _ in range(5):
            batch = random.sample(memory, min(BATCH_SIZE, len(memory)))
            S, R, NS, _ = map(np.array, zip(*batch))
            qn = qt.predict(NS, sess)                                  # shape (B,2)
            tgt = R + DISCOUNT * np.repeat(qn.max(axis=1, keepdims=True), 2, axis=1)
            ql.update(S, tgt.astype("float32"), sess)

        copy_params(sess, ql, qt)

        # update coef: coef ← coef + 0.001*(ep_reward - 0)
    ...
    outdir  = os.path.join("exp", f"AL{AL_budget}_validation")
    ...
    for i in range(K_SLICES):
        ...
        # restrict timeseries to slice i
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(G, P, average="binary", zero_division=0)
        au = average_precision_score(G, P)
        f1s.append(f1)
        aus.append(au)

        prefix = os.path.join(outdir, f"slice_{i}")
        np.savetxt(prefix + ".txt", [precision, recall, f1, au], fmt="%.6f")
