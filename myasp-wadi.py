...
# Convert to numeric, drop any all-NaN column or row
...
print(f"[DATA] Kept {len(df.columns)} numeric columns; {len(df)} rows total.")

# ─── B) Load the attack‐label CSV (may be longer or shorter) ──────────
...
# Truncate sensor df & label array to the same length
min_len = min(len(df), len(anomaly_full))
df = df.iloc[:min_len].reset_index(drop=True)
anomaly = anomaly_full[:min_len]
print(f"[DATA] Truncated both to min_len = {min_len}.")

# ─── C) Build TS DataFrame used by RL ─────────────────────────────────
TS = df.copy()
TS["anomaly"] = anomaly
TS["label"]   = -1
...
windows_full = np.array([
    TS.iloc[i - N_STEPS + 1 : i + 1][feature_cols_full].values.flatten()
    for i in sample_idx
], dtype="float32")

# Drop any column (of that flattened vector) whose variance is zero across all sampled windows
col_std = windows_full.std(axis=0)
keep_mask = col_std > 0
windows = windows_full[:, keep_mask]
feature_cols = [feature_cols_full[i] for i in range(len(feature_cols_full)) if keep_mask[i]]
n_features   = len(feature_cols)
print(f"[VAE] Dropped {len(windows_full[0]) - windows.shape[1]} zero‐var columns; now n_features={n_features}.")
...
INPUT_DIM = windows.shape[1]  # exactly the flattened‐width after dropping zero‐var columns
...
# ─── F) Compute per‐step reconstruction error array ───────────────────
...
print(f"[VAE] Penalty_array length = {len(penalty_array)}; "
      f"stats: min={np.nanmin(penalty_array):.4e}, max={np.nanmax(penalty_array):.4e}, "
...
class EnvWaDi:
    """
    reset() sets t = N_STEPS.  Each step(a) → (t+1).
    Returns (next_state, [r0, r1], done, {}).
    next_state has shape (2, N_STEPS, n_features+1).
    """
    ...
    def reset(self):
        self.t = self.t0
        # Precompute states_list (flattened “action=0” windows) for LP/AL
        self.states_list = []
        for c in range(self.t0, self.N):
            s = self.statefnc(self.timeseries, c)
            if s is not None:
                # store only action=0 case, flattened
...
# ─── Q‐Network definition (TF‐1 style) ─────────────────────────────────
class QNet:
...
# ─── State & Reward functions that use penalty_array ─────────────────
...
# ─── Full train + equal‐slice validate ─────────────────────────────────
def train_and_validate(AL_budget):
    tf.compat.v1.reset_default_graph()
    sess = tf.compat.v1.Session()
    K.set_session(sess)

    # Build two Q‐networks
    ql = QNet("ql"); ql.build()
    qt = QNet("qt"); qt.build()
    ...
    # Optional warm‐up IsolationForest on known‐normal windows
    W0 = []
    env.reset()
    ...
    for ep in range(1, EPISODES + 1):
        # 1) Label‐Propagation + Active Learning
        env.reset()
...
if __name__ == "__main__":
    # You can lower these if you need faster runs
    EPISODES   = 10
    BATCH_SIZE = 64
    DISCOUNT   = 0.5
