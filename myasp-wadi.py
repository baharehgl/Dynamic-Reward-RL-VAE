...
# ─── 1) VAE PRE-TRAIN (EAGER MODE, SMALLER) ────────────────────────
...
MAX_VAE_SAMPLES = 100
sample_idx      = np.random.choice(norm_idx, min(MAX_VAE_SAMPLES,len(norm_idx)), replace=False)
...
# Compute penalty_array in small batches
all_w = np.array([
    df.iloc[i-N_STEPS+1:i+1][feature_cols].values.flatten()
...
# ─── 2) RL LOOP & VALIDATION (TF-1 GRAPH) ──────────────────────────
...
def copy_params(ss,src,dst):
    v1 = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,src.sc)
    v2 = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,dst.sc)
    for a,b in zip(sorted(v1,key=lambda v:v.name), sorted(v2,key=lambda v:v.name)):
        ss.run(b.assign(a))
...
def train_and_validate(AL):
    ...
    for i in range(N_STEPS,len(env.timeseries)):
      if env.timeseries['label'].iat[i]==0:
        buf.append(env.timeseries[feature_cols].iloc[i-N_STEPS+1:i+1].values.flatten())
    if buf:
      from sklearn.ensemble import IsolationForest
      IsolationForest(contamination=0.01).fit(np.array(buf[:MAX_VAE_SAMPLES],'float32'))
    ...
    for ep in range(1,EPISODES+1):
        ...
        print(f"[train AL={AL}] ep{ep:02}/{EPISODES} coef={coef:.2f} reward={ep_reward:.2f}")
    ...
    for i in range(K_SLICES):
        ...
        ts = envv.timeseries_repo[0]
        ts['anomaly'] = ts['anomaly'].values
        ts['label']   = ts['label'].values
        envv.timeseries = ts
        ...
        pre = f"{out}/slice{i}"
        np.savetxt(pre+".txt",[p,r,f1,au],fmt="%.4f")
        fig,ax=plt.subplots(4,sharex=True,figsize=(6,5))
        ...
        plt.tight_layout(); plt.savefig(pre+".png"); plt.close(fig)

        print(f"[val AL={AL}] slice{i+1}/{K_SLICES} F1={f1:.3f} AU={au:.3f}")

    print(f"[val AL={AL}] mean F1={np.mean(f1s):.3f} mean AUPR={np.mean(aus):.3f}\n")
