...
# 1a) Load & clean sensor data (downsample by 2 to speed up)
...
# 1b) Load labels (downsample by 2 to match df)
lbl_all = (
    pd.read_csv(LABEL, header=1)["Attack LABLE (1:No Attack, -1:Attack)"]
    .iloc[::2]
    .astype(int)
    .values
)

# 1c) Sample up to 100 normal windows for VAE training
...
def build_vae(input_dim, hidden=32, latent=5):
    ...
    dec  = models.Model(z_in, out, name="decoder")
...
# 1d) Build full‐length penalty array (one penalty per time index ≥ N_STEPS-1)
all_w = np.array([
    df.iloc[i - N_STEPS + 1 : i + 1][feature_cols].values.flatten()
...
# ── FIX: replace any NaN/∞ with 0.0 ────────────────────────────────────
penalty_array = np.nan_to_num(penalty_array,
                              nan=0.0,
                              posinf=0.0,
                              neginf=0.0)
print("[VAE] penalty_array cleaned, length =", len(penalty_array))
...
class QNet:
    ...
    def build(s):
        with tf.compat.v1.variable_scope(s.sc):
            s.S   = tf.compat.v1.placeholder(
                        tf.float32, [None, N_STEPS, n_features + 1])
...
def copy_params(ss, src, dst):
    v1 = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, src.sc)
    v2 = tf.compat.v1.get_collection(
...
def make_state(ts, c):
    if c < N_STEPS:
        return None
    W = ts[feature_cols].iloc[c - N_STEPS + 1 : c + 1] \
...
def reward_fn(ts, c, a, coef):
    if c < N_STEPS:
        return [0.0, 0.0]
    raw_pen = penalty_array[c]
    # ── NORMALIZE by window size so reward magnitudes stay ~±10 ──
    pen = coef * (raw_pen / float(N_STEPS * n_features))
    if np.isnan(pen):
        pen = 0.0
...
def train_and_validate(AL_budget):
    ...
    for ep in range(1, EPISODES + 1):
        # 1) Label-Propagation + Active Learning at episode start
        env.reset()
        labs = np.array(env.timeseries['label'].iloc[N_STEPS:])
        if np.any(labs != -1):
            W_arr = np.array([s for s in env.states_list if s is not None])
        ...
        while not done:
            ...
            memory.append(T(s, r, ns, done))
            ep_reward += r[a]
            s = ns

        # 4) Replay‐memory updates
        for _ in range(3):
        ...
        # 5) Proportional dynamic‐coef update (bounded [0.1,100])
        coef = max(min(coef + 0.001 * (ep_reward - 0.0), 100), 0.1)
        print(f"[train AL={AL_budget}] ep{ep:02}/{EPISODES}"
              f" coef={coef:.2f} reward={ep_reward:.2f}")

    # ── VALIDATION: equal‐slice (save in exp/ALxxx) ─────────────────────
    # Create output directory
    exp_dir = os.path.join("exp", f"AL{AL_budget}")
    os.makedirs(exp_dir, exist_ok=True)

    # Full timeseries for slicing
    ...
    for i in range(K_SLICES):
        ...
        # Replace timeseries_repo[0] with only the i-th slice
        slice_df = base_ts.iloc[i * seg : (i + 1) * seg].reset_index(drop=True)
        envv.timeseries_repo[0] = slice_df
        ...
        # Save per‐slice performance text
        perf_txt = os.path.join(exp_dir, f"slice_{i}.txt")
        with open(perf_txt, "w") as f:
            f.write(f"Prec={p:.6f}, Recall={r:.6f}, F1={f1:.6f}, AU={au:.6f}\n")

        # Save per‐slice 4-panel plot
        fig, ax = plt.subplots(4, sharex=True, figsize=(8, 6))
        ax[0].plot(V)
        ax[0].set_title("Time Series (last value of each window)")
        ax[1].plot(P, 'g')
        ax[1].set_title("Predictions")
        ax[2].plot(G, 'r')
        ax[2].set_title("Ground Truth")
        ax[3].plot([au] * len(V), 'm')
        ax[3].set_title("AUPR")
        plt.tight_layout()
        fig_path = os.path.join(exp_dir, f"slice_{i}.png")
        plt.savefig(fig_path)
        plt.close(fig)

        print(f"[val AL={AL_budget}] slice{i+1}/{K_SLICES}"
              f" F1={f1:.3f} AU={au:.3f}")

    # Save mean metrics
    mean_txt = os.path.join(exp_dir, "mean_performance.txt")
    with open(mean_txt, "w") as f:
        f.write(f"Mean F1={np.mean(f1s):.6f}, Mean AUPR={np.mean(aus):.6f}\n")

    print(f"[val AL={AL_budget}] mean F1={np.mean(f1s):.3f}"
