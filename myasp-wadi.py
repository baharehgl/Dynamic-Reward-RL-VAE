...
# Anomaly detection on the WADI dataset with VAE-guided RL + AL + LP
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------ paths / GPU
from env_wadi import EnvTimeSeriesWaDi          # custom environment
os.environ['CUDA_VISIBLE_DEVICES'] = "0,1"
gpus = tf.config.list_physical_devices('GPU')
print("GPUs detected by TensorFlow:", gpus)

# ------------------------------ HYPERPARAMETERS & FILE LOCATIONS -------------
EPISODES           = 2
N_STEPS            = 25
DISCOUNT_FACTOR    = 0.5
...
# ───────────────────────── 1) LOAD DATA & KEEP NUMERIC COLUMNS ────────────────
df_all = pd.read_csv(SENSOR_CSV, decimal='.')   # ← CHANGED  (add decimal if needed)

# -- robust numeric coercion instead of blind dropping -----------------  ← CHANGED
df_all.columns = df_all.columns.str.strip()     # trim stray blanks
df_all  = df_all.apply(pd.to_numeric, errors='coerce')
df_all  = df_all.dropna(axis=1, how='all')      # drop truly non-numeric columns
df_all  = df_all.dropna(axis=0, how='all').reset_index(drop=True)

if 'Row' in df_all.columns:                     # optional: Row is just an index
...
# ─────────────────────── 2) PRE-TRAIN VARIATIONAL AUTOENCODER ────────────────
...
sampled    = np.random.choice(normal_pos,
                              size=min(MAX_VAE_SAMPLES, len(normal_pos)),
                              replace=False)
print(f"Sampling {len(sampled)} windows for VAE training...")

# build windows
windows = [df_all.iloc[i-N_STEPS+1:i+1][feature_cols].values.flatten()
           for i in sampled]
...
print(f"Standardised windows shape: {Xs.shape}\n")
...
def reward_fn(ts, c, a, vae_model, coef):
    if c < N_STEPS:
        return [0, 0]
    win = ts[feature_cols].iloc[c-N_STEPS+1:c+1].values.flatten() \
    ...
    def __init__(self, scope, lr=3e-4):
        ...
        with tf.compat.v1.variable_scope(scope):
            self.S = tf.compat.v1.placeholder(
                tf.float32, [None, N_STEPS, N_INPUT_DIM], 'S')
            self.T = tf.compat.v1.placeholder(
                tf.float32, [None, ACTION_SPACE_N], 'T')
...
def copy_params(sess, src, dst):
    vs = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                                     scope=src.scope)
    vt = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                                     scope=dst.scope)
    for s, t in zip(sorted(vs, key=lambda v: v.name),
                    sorted(vt, key=lambda v: v.name)):
        sess.run(t.assign(s))
...
def q_learning(env, sess, ql, qt, vae_model, init_coef):
    ...
    for i in range(N_STEPS, len(env.timeseries)):
        if env.timeseries['label'].iat[i] == 0:
            W.append(env.timeseries[feature_cols]
                     .iloc[i-N_STEPS+1:i+1].values.flatten())
    if len(W):
        IsolationForest(contamination=0.01).fit(
            np.array(W[:MAX_VAE_SAMPLES], dtype='float32'))
    ...
    for ep in range(EPISODES):
        ...
        env.rewardfnc = lambda ts, c, a, coef=dynamic_coef: \
                        reward_fn(ts, c, a, vae_model, coef)
        ...
        for u in uncert[:NUM_AL]:
            env.timeseries['label'].iat[u + N_STEPS] = \
                env.timeseries['anomaly'].iat[u + N_STEPS]
    ...
    with open('validation/perf.txt', 'w') as fout:
        for i in range(EPISODES):
            ...
            while not done:
                ...
                preds.append(a)
                gts.append(env.timeseries['anomaly']
                           .iat[env.timeseries_curser])
            ...
            fout.write(f"E{i+1}:P={p:.3f},R={r:.3f},F1={f1:.3f},"
                       f"AUPR={aupr:.3f}\n")
            f1s.append(f1)
            auprs.append(aupr)
    print(f"Validation complete: Avg F1={np.mean(f1s):.3f}, "
          f"Avg AUPR={np.mean(auprs):.3f}")
...
def train_wrapper(nLP, nAL, discount):
    ...
    f1, aupr       = validate(
        EnvTimeSeriesWaDi(SENSOR_CSV, LABEL_CSV, N_STEPS), sess, trained)

    exp = f'exp_AL{nAL}'
    os.makedirs(os.path.join(exp, 'plots'), exist_ok=True)
    plt.figure()
    plt.plot(coefs)
    plt.title('dynamic_coef')
    plt.savefig(os.path.join(exp, 'plots', 'coef.png'))
