...
# ------------------------------------------------------------------------------
# Get the current directory of this file (for relative paths).
current_dir = os.path.dirname(os.path.abspath(__file__))

# Build relative paths to normal-data and A1Benchmark directories.
# Make sure these folders are in the same directory as this script.
normal_data_dir = os.path.join(current_dir, "normal-data")
benchmark_dir = os.path.join(current_dir, "ydata-labeled-time-series-anomalies-v1_0", "A1Benchmark")
# ------------------------------------------------------------------------------
...
def load_normal_data(data_path):
    """
    Load all CSV files from the given directory, concatenate, and scale them.
    """
    all_files = [
        os.path.join(data_path, fname)
        for fname in os.listdir(data_path) if fname.endswith('.csv')
    ]
...
# Use the relative path for normal data
...
# Constants for VAE
original_dim = 3  # Adjust if your data has more columns
latent_dim = 10
intermediate_dim = 64
...
# The state function returns a vector of shape (n_steps, n_input_dim).
...
# Reward function with adaptive scaling factor (tau) and VAE penalty
tau_values = []
...
def adaptive_scaling_factor_dro(preference_strength,
...
class Q_Estimator_Nonlinear():
    ...
    def __init__(self, learning_rate=np.float32(0.01), scope="Q_Estimator_Nonlinear", summaries_dir=None):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            self.action_values = (tf.matmul(self.outputs[-1], self.weights['out'])
                                  + self.biases['out'])
    ...
    def update(self, state, target, sess=None):
        ...
        summaries, _, _ = sess.run(
            [self.summaries, tf.compat.v1.train.get_global_step(), self.train_op],
            feed_dict
        )
        if self.summary_writer:
            self.summary_writer.add_summary(summaries, tf.compat.v1.train.get_global_step())
        return
...
def q_learning(env,
    ...
    total_t = tf.compat.v1.train.get_global_step().eval(session=sess)
    ...
    # --------------------------------------------------------------------------
    # Populate replay memory with initial experience by SVM or Isolation Forest
    popu_time = time.time()
    ...
    # Warm-up model (e.g., IsolationForest)
    ...
    for t in itertools.count():
        ...
        label_list = [-1] * len(state_list)  # unlabeled initially
    ...
    for i_episode in range(num_episodes):
        ...
        al = active_learning(env=env, N=num_active_learning,
...
class active_learning(object):
    ...
    def label(self, active_samples):
        # Example of interactive labeling (not often used in practice)
...
class WarmUp(object):
    ...
    def warm_up_isolation_forest(self, outliers_fraction, X_train):
        """
        Fit an IsolationForest using a 1D transformation of states. Adjust as needed for multi-dimensional data.
        """
...
def train(num_LP, num_AL, discount_factor, learn_tau=True):
    """
    Train the RL agent with learned tau using DRO or a default reward function.
    """
    ...
    # Use the relative path for dataset directory
