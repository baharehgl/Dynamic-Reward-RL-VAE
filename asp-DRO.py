...
# If you keep your environment folder in the parent directory:
...
def load_normal_data(data_path):
    """
    Load CSV files from `data_path`, concatenate, and scale the numeric values.
    """
    all_files = [
...
def RNNBinaryStateFuc(timeseries, timeseries_curser, previous_state=[], action=None):
    """
    Returns a vector (or 2 possible states) for our RNN of shape (n_steps, n_input_dim).
    """
...
def RNNBinaryRewardFuc(timeseries, timeseries_curser, action=0, vae=None, scale_factor=10):
    """
    Reward = tau * (BaseReward + scaled reconstruction penalty)
    """
    if timeseries_curser >= n_steps:
        ...
        preference_strength = np.clip(
            1 / (1 + np.exp(-reconstruction_error)),
            0.05,
            0.95
        )
        ...
        else:  # timeseries['label'][timeseries_curser] == 1
            return [
...
def RNNBinaryRewardFucTest(timeseries, timeseries_curser, action=0):
    """
    Reward for testing/evaluation: use 'anomaly' column for ground truth.
    """
    if timeseries_curser >= n_steps:
...
class Q_Estimator_Nonlinear:
    """
    RNN-based Q(s,a) approximator using TensorFlow (compat.v1).
    """
    def __init__(self, learning_rate=np.float32(0.0003),
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            # IMPORTANT: pass global_step to auto-increment:
...
def make_epsilon_greedy_policy(estimator, nA):
    """
    Creates an epsilon-greedy policy based on Q-values and epsilon.
    """
...
def q_learning(env,
               ...
               global_step=None):
    """
    Q-Learning with function approximation (RNN).
    """
    ...
    # Start from existing global_step value
    ...
    ##########################################################################
    # 4.1) Warm up replay memory with initial outlier-based labeling
    ##########################################################################
    ...
    for t in itertools.count():
        ...
        for sample in warm_samples:
            ...
            if env.timeseries_curser >= len(env.timeseries):
                # If the curser goes out of range, skip
                continue
            # ------------------------------------------------------------
    ...
    for i_episode in range(num_episodes):
        ...
        while env.datasetidx > env.datasetrng * validation_separate_ratio:
            # If the chosen dataset is for validation, reset again
            env.reset()
            print('double reset')
        ...
        # For all labeled indices, do Q-learning steps
        ...
        for idx in certainty_index:
            if (n_steps + idx) < len(env.timeseries):
                pseudo_label = lp_model.transduction_[idx]
                env.timeseries['label'][n_steps + idx] = pseudo_label
        ...
        for _ in range(num_epoches):
            ...
            if qlearn_estimator.summary_writer:
                ...
                # e.g., episode_summary.value.add(tag='some_tag', simple_value=some_value)
                qlearn_estimator.summary_writer.add_summary(episode_summary, current_gstep)

            # Target estimator sync
...
def q_learning_validator(env, estimator, num_episodes, record_dir=None, plot=1):
    """
    Validate the trained model, collecting y_true vs y_pred across multiple episodes.
    """
    y_true_all = []
    y_pred_all = []
    ...
    for i_episode in range(num_episodes):
        state = env.reset()
        # Make sure we are in the test portion (some environment code has separate sets)
        while env.datasetidx < env.datasetrng * validation_separate_ratio:
            state = env.reset()
...
class active_learning(object):
    ...
    def label(self, active_samples):
        # Example of interactive labeling; not typically used in batch script
...
class WarmUp(object):
    """
    Used to do an outlier detection based warm-up (OneClassSVM, IsolationForest, etc.)
    """
...
def train(num_LP, num_AL, discount_factor, learn_tau=True):
    """
    Train the RL agent with dynamic tau (or not).
    """
    # Example: Train VAE from scratch
    vae, _ = build_vae(original_dim, latent_dim, intermediate_dim)
    vae.fit(x_train, epochs=5, batch_size=32)  # Keep epochs small for quick test
    vae.save('vae_model.h5')

    # Or load an existing VAE:
    ...
    dataset_dir = [benchmark_dir]  # your time-series anomaly folder
    ...
    for i in range(len(dataset_dir)):
        ...
        with sess.as_default():
            q_learning(
                ...
                num_episodes=30,          # For demonstration, you can increase
                num_epoches=10,
