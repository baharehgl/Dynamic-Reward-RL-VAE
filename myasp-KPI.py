...
def build_and_train_vae():
    x = layers.Input((n_steps,))
    h = layers.Dense(inter_dim, activation='relu')(x)
    ...
    recon = losses.mse(x, x_dec)*n_steps
...
def reward_fn(ts, cur, action, lam):
    win = np.array([ts['value'].iloc[cur-n_steps:cur]])
    err = np.mean((vae_model.predict(win)-win)**2)
    pen = lam * err
...
class ActiveLearner:
    def __init__(self, env, N, est):
        self.env = env; self.N=N; self.est=est
...
def train_with_validation(train_csv, test_csv):
    ...
    for ep in range(1, EPISODES+1):
        ...
        # logging & lam update
        total_r = sum(r[a] for (s,r,ns,d) in mem for a in [0])
    ...
    plt.figure(); plt.plot(rewards); plt.title("Reward"); plt.savefig("exp/reward.png"); plt.close()
    plt.figure(); plt.plot(lams);    plt.title("Lambda"); plt.savefig("exp/lambda.png"); plt.close()
